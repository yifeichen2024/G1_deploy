...
Remember: Be helpful and accurate, but KEEP IT SHORT. Always confirm the complete order before submitting."""
    ...
    async def monitor_file(self, file_path = "/home/unitree/G1_deploy/l2_trigger_state.txt", poll_interval: float = 1.0):
        ...
        last_seen = None
        while True:
            if self.system_call == True and self.system_call != last_seen:
                last_seen = self.system_call
    ...
    async def system_call_update(self, system_call):
        s
...
def main():
    ...
    try:
        ...
        ctrl.start()                 # 启动线程
        asyncio.create_task(asyncio.to_thread(ctrl.remote_poll_audio()))
        ...
        async def system_call_update():
            while True:
                assistant.system_call = ctrl.ready2placebill
                
        asyncio.create_task(asyncio.to_thread(system_call_update()))
