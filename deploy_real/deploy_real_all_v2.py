#!/usr/bin/env python3
"""
G1 deploy script  (re-architected)

  • Motion-mode  : 多个 ONNX 动作策略
  • Loco-mode    : TorchScript 行走策略
  • Zero-torque → Default pose → 等待 A → Stance
  • 热键同旧版:  L1/R1 切动作 | L2+Y 进 Loco | R2+Y 回 Stance
"""
...
class PolicyOutput:
    def __init__(self, nj: int):
        self.actions = np.zeros(nj, dtype=np.float32)
        self.kps     = np.zeros(nj, dtype=np.float32)
        self.kds     = np.zeros(nj, dtype=np.float32)
...
class _State:
    ...
    def __init__(self, nj: int):
        ...
        self.vel_cmd     = np.zeros(3,  np.float32)

# ════════════════════════════════════════════════════════════════
# 2.  Motion-mode ONNX policy (与旧版相同)
# ════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════
# 3.  Loco-mode TorchScript policy (与旧版相同)
# ════════════════════════════════════════════════════════════════
class LocoTorchPolicy:
    def __init__(self, full_joints: int, loco_yaml: str):
        ...
        self.cfg = cfg
        self.nj  = full_joints
        self.joint2motor = np.array(cfg["action_joint2motor_idx"], dtype=np.int32)
    ...
    def step(self, st: _State) -> PolicyOutput:
        ...
        cmd_sc = self._scale_cmd(st.vel_cmd) * cfg["cmd_scale"]
        ...
        self.obs[9+2*self.na:]            = self.action_prev

        with torch.inference_mode():
            act = self.ts(torch.from_numpy(self.obs).unsqueeze(0)).clip(-100,100).squeeze().cpu().numpy()
        self.action_prev = act

        full_tgt = self.def_ang.copy()
...
class Controller:
    def __init__(self, motion_cfg: Config, loco_yaml: str):
        ...
        self.nj     = motion_cfg.num_actions                           # G1 全指数
        # ── DDS init ───────────────────────────────────────────
        ...
        for i, p in enumerate(motion_cfg.policy_paths):
            name = motion_cfg.policy_names[i] if hasattr(motion_cfg, "policy_names") else f"motion{i}"
    ...
    def _handle_buttons(self):
        ...
        # L2+Y → Loco
        if btn[KeyMap.L2] and btn[KeyMap.Y] and self.idx != self.loco_idx and now - self.last_switch > self.cooldown:
            self.idx = self.loco_idx ; self.last_switch = now ; print("< LOCOMODE >")
        # R2+Y → Stance
        if btn[KeyMap.R2] and btn[KeyMap.Y] and self.idx == self.loco_idx and now - self.last_switch > self.cooldown:
    ...
    def _send(self, po: PolicyOutput):
        ...
        for j, m in enumerate(self.cfg.fixed_joint2motor_idx):
            mc = self.low_cmd.motor_cmd[m]
            mc.q, mc.qd, mc.kp, mc.kd, mc.tau = self.cfg.fixed_target[j], 0, self.cfg.fixed_kps[j], self.cfg.fixed_kds[j], 0
    ...
    def loop_once(self) -> bool:
        ...
        if self.idx not in (0, self.loco_idx):
            ...
            if pol.step_counter * pol.dt >= pol.len_s:
                print("[INFO] Motion done -> stance")
                self.idx = 0 ; pol.step_counter = 0
        return False
...
if __name__ == "__main__":
    pa = argparse.ArgumentParser("Motion + Loco deploy (refactored)")
